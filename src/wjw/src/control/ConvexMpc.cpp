...
void ConvexMpc::calculate_B_mat_c(double robot_mass, const Eigen::Matrix3d &a1_trunk_inertia, Eigen::Matrix3d root_rot_mat,
    ...
    for (int i = 0; i < NUM_LEG; ++i) {
        B_mat_c.block<3, 3>(6, 3 * i) =
                a1_trunk_inertia_world.inverse() * skew(foot_pos.block<3, 1>(0, i));
...
void ConvexMpc::calculate_qp_mats( Eigen::Matrix<double, MPC_STATE_DIM, 1>  mpc_states, 
...
//    dense_hessian = (B_qp.transpose() * Q * B_qp + R);
    ...
    // calculate gradient
    Eigen::Matrix<double, MPC_STATE_DIM*PLAN_HORIZON, 1> tmp_vec = A_qp* mpc_states;
