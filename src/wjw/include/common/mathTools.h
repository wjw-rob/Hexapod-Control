...
// 这段代码是一个模板函数，名为`saturation`，用于将输入值 `a` 限制在给定的范围内。它的参数如下：
// - `a`: 要进行饱和处理的值。
// - `limits`: 一个包含两个元素的 `Vec2`，表示上下限。这里假设 `Vec2` 是一个包含两个元素的向量或者数组。
// 函数首先确定上下限，并确保 `lowLim` 存储较小的值，`highLim` 存储较大的值。
//然后，它检查输入值 `a` 是否小于 `lowLim`，如果是，则返回 `lowLim`；如果 `a` 大于 `highLim`，则返回 `highLim`；否则返回 `a`。
// 这个函数的作用类似于数学上的取值范围限制函数，确保值在指定范围内。
template<typename T>
...
// 这段代码是一个模板函数，名为`invNormalize`，用于将输入值 `value` 从一个范围映射到另一个范围。它的参数如下：
// - `value`: 要进行映射的值。
// - `min`: 目标范围的最小值。
// - `max`: 目标范围的最大值。
// - `minLim`: 源范围的最小值，默认为 -1。
// - `maxLim`: 源范围的最大值，默认为 1。
// 这个函数的作用是将一个值从一个范围映射到另一个范围，常用于数据归一化或标准化的处理中。
...
/*这段代码是一个将旋转矩阵（RotMat 或 Mat3）转换为欧拉角（Roll, Pitch, Yaw）的函数，
常用于从旋转矩阵中提取出机器人的姿态。具体来说，它计算了三个角度：Roll（滚转）、Pitch（俯仰）、Yaw（偏航）*/
inline Vec3 rotMatToRPY(const Mat3& R) {
    Vec3 rpy;
    rpy(0) = atan2(R(2,1),R(2,2));  //x
    rpy(1) = asin(-R(2,0));         //y
    rpy(2) = atan2(R(1,0),R(0,0));  //z
    return rpy;
}
...
class AvgCov{
public:
    AvgCov(unsigned int size, std::string name, bool avgOnly=false, unsigned int showPeriod=500, unsigned int waitCount=5000, double zoomFactor=1000000)
